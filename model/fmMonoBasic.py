...
# the decimation rate when reducing the front end sampling rate (i.e., RF) to a smaller samping rate at the
# intermediate frequency (IF) where the demodulated data will be split into the mono/stereo/radio data channels
rf_decim = 10
rf1_decim = 5
...
def convolution(h,x):
	y= np.zeros(len(x)) #can not be len(h)+len(x)-1 because in filtered_single_pass, the space allocated
						#for filtered data is filtered_data = np.empty(shape = audio_data.shape) which is
						#exactly the size of the audio data. No more no less. Even though when convolution
						#happens, it increases the output size to x+h-1, but the allocated data only allows
						#the convolved signal to be exactly the right length
...
if __name__ == "__main__":
	...
	if il_vs_th == 0:
		...
		audio_coeff =my_own_coeff(rf1_Fc,if_Fs,rf_taps)
	...
	audio_data = audio_filt[::5]
	#audio_data = signal.decimate(audio_filt, 5)# to be updated by you during in-lab (same code for takehome)
